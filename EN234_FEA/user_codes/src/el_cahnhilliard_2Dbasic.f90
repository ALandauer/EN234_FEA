...
subroutine el_cahnhilliard_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real( prec ), intent( inout )    :: dof_increment(length_dof_array)                        ! DOF increment, stored as du1,du2,du3,du4... for each node in turn
...
!    real (prec)  ::  c(n_nodes), dc(n_nodes)              ! concentration
!    real (prec)  ::  mu(n_nodes), dmu(n_nodes)            !
    ...
    real (prec)  ::  q(6),del_dmuc(6),dmuc(6),ttl_muc(6),ttl_muct(6) !
    real (prec)  ::  diffusion, kappa, theta               ! Material properties
    ...
    diffusion = element_properties(1)
    kappa = element_properties(2)
    theta = element_properties(3)

!    muc(1:2*n_nodes-2:2) = mu(1:n_nodes)
!    muc(2:2*n_nodes-1:2) = c(1:n_nodes)
    ...
    do kint = 1, n_points
        ...
        dmuc(1:6) = matmul(B,dof_total)
        del_dmuc(1:6) = matmul(B,dof_increment)
        ttl_muc(1:6) = matmul(B,dof_total+dof_increment)

!        dof_increment(1:2*n_nodes-2:2) = theta*dof_increment(1:2*n_nodes-2:2)

        ttl_muct(1:6) = matmul(B,dof_total+theta*dof_increment)

        q(1) = dmuc(1) + del_dmuc(1) - ((dmuc(2)+del_dmuc(2))*((dmuc(2)+del_dmuc(2))**2.d0 - 1.d0))
        q(2) = del_dmuc(2)/DTIME
        q(3) = -kappa*ttl_muc(5)
        q(4) = -kappa*ttl_muc(6)
        q(5) = diffusion*ttl_muct(3)
        q(6) = diffusion*ttl_muct(4)

        D(1,1) = 1.d0
        D(1,2) = -1*(3.d0*(ttl_muc(2))**2.d0-1.d0)
        D(2,2) = 1.d0/DTIME
        D(3,5) = -kappa
        D(4,6) = -kappa
        D(5,3) = theta*diffusion
        D(6,4) = theta*diffusion
    ...
    element_residual = -1*element_residual
